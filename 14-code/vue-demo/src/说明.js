 ...
 * 
 * 
 *  2.Vue中的混合:把某些组件中共同的配置的内容抽取出来,放在一个js文件中,在组件中引入,并注册混合就可以使用了,我们不是很常用
 * 
 * 
 *  3. 事件:
 *  Vue中事件分类:
 ...
 * 
 * 路由守卫:全局路由守卫和组件内的路由守卫(局部路由守卫)
 * 路由守卫:守护路由的,当点击链接向某个地址进行跳转的时候,可以对目标的地址进行判断,或者说对将要离开的地址进行判断(可以获取到将要跳转到的路由的地址,也可以获取将要离开的路由的地址),通过判断,可以限制路由的跳转(可以设置路由的跳转地址)
 * 全局路由守卫:在创建路由器的时候,通过beforeEach方法进行路由的限定(在router.js中进行书写的)
 * 组件内的路由守卫:在组件内部是一个钩子(回调函数),在组件内部使用的,组件内部是没法发直接获取当前组件实例对象的,必须要在这个路由守卫的next方法中传入的回调函数内部中获取当前路由组件实例对象
 * 全局路由守卫是对整个项目的路由进行守卫操作,而组件内的路由守卫,守卫的是当前的组件
 * 
 * 
 * 
 * 
